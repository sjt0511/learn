...
ES6 模块的设计思想是**尽量的静态化**，使得**编译时就能确定模块的依赖关系，以及输入和输出的变量**。CommonJS 和 AMD 模块，都只能在运行时确定这些东西。比如，CommonJS 模块就是对象，输入时必须查找对象属性
...
- `import`命令输入的**变量都是只读的**，因为它的本质是输入接口
...
- `import`语句会执行所加载的模块，所以模块可以不需要任何输出
- 多次重复执行同一句`import`语句，那么只会执行一次，而不会执行多次(加载多次但只执行一次)
- `import`语句是 Singleton 模式
- 通过 Babel 转码，CommonJS 模块的`require`命令和 ES6 模块的`import`命令，可以写在同一个模块里面。但由于`import`是在静态解析阶段执行，是模块中最早执行的，不推荐写在同一模块里
...
`import`命令输入的**变量都是只读的**，因为它的本质是输入接口。也就是说，不允许在加载模块的脚本里面，改写接口
